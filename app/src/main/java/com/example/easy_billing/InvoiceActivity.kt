...
class InvoiceActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        invoiceAdapter = InvoiceAdapter(
            ...
            gstScheme   = gstScheme,
            onItemClick = { item -> showLineDiscountDialog(item) }
        )
    ...
    private fun showLineDiscountDialog(item: CartItem) {
        ...
        val presets = listOf(
            v.findViewById<TextView>(R.id.tvPreset1),
            v.findViewById<TextView>(R.id.tvPreset2),
            v.findViewById<TextView>(R.id.tvPreset3),
            v.findViewById<TextView>(R.id.tvPreset4)
        )
        val tvOrig = v.findViewById<TextView>(R.id.tvDiscOrig)
        val tvNew  = v.findViewById<TextView>(R.id.tvDiscNew)
        val tvSave = v.findViewById<TextView>(R.id.tvDiscSave)

        val gross = GstBillingCalculator.round2Pub(item.product.price * item.quantity)
        val qtyStr = if (item.quantity % 1 == 0.0) item.quantity.toInt().toString()
                     else item.quantity.toString()
        v.findViewById<TextView>(R.id.tvDiscProduct).text = item.product.name
        v.findViewById<TextView>(R.id.tvDiscLineTotal).text =
            "$qtyStr × ${CurrencyHelper.format(this, item.product.price)} = ${CurrencyHelper.format(this, gross)}"

        tvOrig.paintFlags = tvOrig.paintFlags or android.graphics.Paint.STRIKE_THRU_TEXT_FLAG

        var isPercent = true
        val pctPresets = listOf(5, 10, 15, 20)
        val amtPresets = listOf(10, 25, 50, 100)

        fun currentAmount(): Double {
            val raw = et.text.toString().toDoubleOrNull() ?: 0.0
            val amt = if (isPercent) gross * raw / 100.0 else raw
            return GstBillingCalculator.round2Pub(amt.coerceIn(0.0, gross))
        }
        fun refreshPreview() {
            val d = currentAmount()
            tvOrig.text = CurrencyHelper.format(this, gross)
            tvNew.text  = CurrencyHelper.format(this, GstBillingCalculator.round2Pub(gross - d))
            val pct = if (gross > 0) d / gross * 100.0 else 0.0
            val pctStr = if (pct % 1.0 == 0.0) pct.toInt().toString() else String.format("%.1f", pct)
            tvSave.text = "${CurrencyHelper.format(this, d)} · $pctStr%"
        }
        fun styleToggle() {
            val on = modePct.takeIf { isPercent } ?: modeAmt
            val off = if (isPercent) modeAmt else modePct
            on.setBackgroundResource(R.drawable.bg_disc_toggle_on); on.setTextColor(0xFFFFFFFF.toInt())
            off.setBackgroundColor(0x00000000); off.setTextColor(0xFF8A8272.toInt())
        }
        fun applyMode(pct: Boolean) {
            isPercent = pct
            // Percent → suffix on the right; Amount → ₹ prefix on the left.
            unitSuffix.visibility = if (pct) View.VISIBLE else View.GONE
            unitPrefix.visibility = if (pct) View.GONE else View.VISIBLE
            val vals = if (pct) pctPresets else amtPresets
            presets.forEachIndexed { i, p ->
                p.text = if (pct) "${vals[i]}%" else CurrencyHelper.format(this, vals[i].toDouble())
            }
            styleToggle(); refreshPreview()
        }
        ...
        presets.forEachIndexed { i, p ->
            p.setOnClickListener {
                val vals = if (isPercent) pctPresets else amtPresets
                et.setText(vals[i].toString())
        ...
        if (item.discountAmount > 0.0) {
            applyMode(false)
            et.setText(GstBillingCalculator.round2Pub(item.discountAmount).let {
        ...
        v.findViewById<View>(R.id.btnApplyDiscount).setOnClickListener {
            item.discountAmount = currentAmount()
            recalculate()
            dialog.dismiss()
        }
        v.findViewById<View>(R.id.btnRemoveDiscount).setOnClickListener {
