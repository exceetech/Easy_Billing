...
class PeakHoursFragment : Fragment(R.layout.fragment_peak_hours), Filterable {
    ...
    private fun showChartDialog(data: List<PeakHourResponse>) {
        ...
        chart.axisLeft.apply {
            ...
            axisMinimum = -(maxValue * 0.10f)
            axisMaximum = maxValue * 1.18f      // headroom so top bubble isn't clipped
            setDrawAxisLine(true)
            axisLineColor = Color.parseColor("#D1D5DB")
            axisLineWidth = 1.5f
            setDrawGridLines(true)
        ...
        chart.setExtraOffsets(53.5f, 16f, 8f, 12f) // left padding (~0.5cm more) so "12am" sits further right
        chart.animateY(900)
        chart.invalidate()
