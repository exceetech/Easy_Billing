...
object GstEngine {
    ...
    /**
     * The statutory GSTIN shape: 2-digit state code, 5-letter PAN prefix,
     * 4 digits, 1 letter, 1 entity digit/letter, literal 'Z', 1 checksum
     * character. Same pattern the GSTR-1 validator enforces, shared here so
     * a bad GSTIN is caught at entry instead of at filing time.
     */
    private val GSTIN_REGEX =
        Regex("^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}Z[0-9A-Z]{1}$")

    /** True only for a structurally valid 15-character GSTIN. */
    fun isValidGstin(gstin: String?): Boolean {
        val g = gstin?.trim()?.uppercase() ?: return false
        return GSTIN_REGEX.matches(g) && INDIA_STATES.containsKey(g.substring(0, 2))
    }
